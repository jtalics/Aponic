...
public class MidiDeviceGetter {
	...
	public static void listReceiverDevices() throws MidiUnavailableException {
		...
		for (int i = 0; i < infos.length; i++) {
			...
			if (device.getMaxReceivers() != 0)
				System.out.println("'"+device.getDeviceInfo().getName().toString() + "' has receivers");
		}
	}
	...
	public static MidiDevice getInputDevice(String deviceName) throws MidiUnavailableException {
		MidiDevice.Info[] infos = MidiSystem.getMidiDeviceInfo();
		for (int i = 0; i < infos.length; i++) {
			MidiDevice device = MidiSystem.getMidiDevice(infos[i]);
			if (device.getMaxTransmitters() != 0 && device.getDeviceInfo().getName().equals(deviceName)) {
				System.out.println("'"+device.getDeviceInfo().getName().toString() + "' was chosen because it has a transmitter and correct name: "+deviceName);
				return device;
			}
		}
		return null;
	...
	public static MidiDevice getOutputDevice(String deviceName) throws MidiUnavailableException {
		MidiDevice.Info[] infos = MidiSystem.getMidiDeviceInfo();
		for (int i = 0; i < infos.length; i++) {
			MidiDevice device = MidiSystem.getMidiDevice(infos[i]);
			if (device.getMaxReceivers() != 0 && device.getDeviceInfo().getName().equals(deviceName)) {
				System.out.println("'"+device.getDeviceInfo().getName().toString() + "' was chosen because it has a receiver and correct name: "+deviceName);
				return device;
			}
		}
		return null;
...
//System.exit(0);
		inputDevice = MidiDeviceGetter.getInputDevice("USB Audio Device");
		...
		// opening the device
		System.out.println("open inputDevice: " + inputDevice.getDeviceInfo().toString());
		inputDevice.open();
		...
		// keyboard
		System.out.println("close inputDevice: " + inputDevice.getDeviceInfo().toString());
		inputDevice.close();
...
class DumpReceiver implements Receiver {
	...
	public static String getHexString(byte[] aByte) {
		...
		for (int i = 0; i < aByte.length; i++) {
			...
			/*
			 * byte bhigh = (byte) ((aByte[i] & 0xf0) >> 4); sbuf.append((char) (bhigh
			 * > 9 ? bhigh + 'A' - 10: bhigh + '0')); byte blow = (byte) (aByte[i] &
			 * 0x0f); sbuf.append((char) (blow > 9 ? blow + 'A' - 10: blow + '0'));
			 */
		}
